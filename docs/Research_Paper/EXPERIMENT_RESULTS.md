...
**Status:** Canonical results ledger for `docs/Research_Paper/`. Record infrastructure and model metrics separately; never fold infrastructure misses into model-negative counts. For the current SoM architecture state, residual-routing program, and live hard-run results, see [`docs/Research_Paper/SoM/EXPERIMENT_RESULTS.md`](./SoM/EXPERIMENT_RESULTS.md) and [`docs/Research_Paper/Ducky/DR_DUCKY_RESEARCH_PAPER.md`](./Ducky/DR_DUCKY_RESEARCH_PAPER.md). The publishable main theorem-search baseline remains `EXP-058` at `1277/2000`, with validated offline residual extension to `~1371/2000`. Dr. Ducky metrics should now always be reported in the split form: routing, theorem-faithful replay, certificate generation, projector compilation, progress lift, and closure lift. Integrated bridge metrics should additionally split `Ducky1`, `first_order_search`, `controller_mode`, `Ducky2`, and `rarified_gap_packet` outcomes.
...
### EXP-APPLY-047: Static Type-Shape Compatibility Filter + Probe Collection

**Date:** 2026-03-19
**Script:** `scripts/run_apply047_compat_filter.py`
**Purpose:** Test whether a static type-shape compatibility filter can cheaply prune the apply candidate pool while preserving gold, and collect probe data for training an executable selector.

**Setup:** 107 eval examples, 91 started (85.0%), 76 gold in scope (83.5%).

**Results:**

| Metric | Value |
|---|---|
| Candidates before filter | 10 (mean) |
| Candidates after filter | 4.1 (mean, 59.5% reduction) |
| Gold survival through filter | 94.7% |
| LeanAccepted cosine_top1 | 15/91 (16.5%) |
| LeanAccepted compat_cosine | 16/91 (17.6%) |
| LeanAccepted compat_reranker | 13/91 (14.3%) |
| **top5_probe_window** | **31/91 (34.1%)** |

- `top5_probe_window` = best of filtered top-5 via Lean verify — probe ceiling within the 5-candidate window, not a true oracle. Selector can legitimately beat it by using a wider window.
- Dominant failure: `unification_mismatch` 199/233 probes (85.4%). Static filter too coarse.
- Static filter adds nothing as standalone selector; keep as feature + soft pruning, not hard gate.
- Generated 9,231 `(goal, candidate)` probe rows → `data/apply_exec_train.jsonl`.

**Decision:** Static filter is a feature, not a gate. Proceed to learned executable selector.

---

### EXP-048: ExecSelector v1

**Date:** 2026-03-19
**Script:** `scripts/run_exp048_exec_selector.py`
**Purpose:** Train and evaluate a binary executable selector: given (goal, candidate), predict P(Lean accepts `apply candidate`).
**Architecture:** Pointwise MLP `[goal_emb(384); cand_emb(384); cosine_score; filter_passed] → P(executable)`.
**Training:** 9,231 step-0 apply train probes, pos_weight=9.2 (1:9 imbalance). Val PR-AUC: 0.713.

**Results (live Lean, 91 goals):**

| Condition | Acc\|started | Acc\|gold_in_scope |
|---|---|---|
| cosine_top1 | 15/91 (16.5%) | 15/76 (19.7%) |
| filter_cosine_top1 | 15/91 (16.5%) | 15/76 (19.7%) |
| **selector_top1** | **35/91 (38.5%)** | **34/76 (44.7%)** |
| top5_probe_window | 31/91 (34.1%) | 31/76 (40.8%) |

- selector_top1 searches wider filtered top-20; beating top5_probe_window is expected and correct.
- **Claim: selector improves valid apply-step selection (+20 goals vs cosine, Δ=+133%).**
- This is NOT yet theorem-level improvement. 34/35 wins are `accepted_with_goals` — productive subgoals opened, not closed goals.
- Static filter adds 0 over cosine as standalone selector. Deploy only as input feature.
- Checkpoint: `models/apply_exec_selector_v1.pt`.

**Decision (frozen):** ExecSelector v1 is the production apply selector for step-0 canonical goals. The missing interface is controller → specialist trigger.
...
**Date:** 2026-03-19
**Script:** `scripts/run_exp049_selector_search.py`
**Purpose:** Test whether the ExecSelector's step-level improvement transfers to theorem-level proof success.
**Setup:** 50-theorem Mathlib benchmark (same split as EXP-3.2), `search_mode=no_learned`, IB baseline.

**Conditions:**

| Condition | Description |
|---|---|
| baseline | IB only (no cosine_apply) |
| cosine_apply | IB + cosine_apply_gated (raw cosine, beam=5) |
| selector_apply | IB + cosine_apply + ExecSelector v1 (beam=1) |

**Results:**

| Condition | Proved | Apply-lane closes | Time |
|---|---|---|---|
| baseline (IB only) | 13/50 (26%) | 0 | 48s |
| cosine_apply | 14/50 (28%) | 0 | 20s |
| selector_apply | 14/50 (28%) | 0 | 75s |

**Key findings:**

1. **Apply gate fired zero times across both apply conditions.** `_apply_gate` requires IB to have failed on a single open goal — a condition not met before other lanes closed first.
2. The +1 theorem (`WeierstrassCurve.Jacobian`) came from `cosine_rw`, not apply.
3. selector_apply 3.8× slower than cosine_apply due to per-theorem model reload (fixed by pre-loading; not real selector cost).
4. **Boundary crossed**: executor validated (EXP-048), orchestration/trigger unresolved (EXP-049).

**Architectural conclusion:** The apply selector is not the bottleneck. The bottleneck is the system never reaching a state where apply is triggered. The next learned object is: *when* should the controller invoke the apply specialist. This is a SoM orchestration question, not a local-executor question.

**Decision (frozen):** Do not relax `_apply_gate` ad hoc. Train a trigger model on live search residuals.

---

### EXP-050: Apply Trigger Dataset Collection

**Date:** 2026-03-19
**Script:** `scripts/collect_trigger_states.py` + `scripts/collect_trigger_parallel.sh`
**Purpose:** Collect large-scale ground-truth trigger labels for training a controller-level apply-trigger classifier.

**Design:**

Four trigger checkpoints in the search trace:
...
- `mid_search` — any open goal after step ≥ 5 with no recent progress

**Label schema (corrected from circular proxy):**
- `can_apply` — whether ANY candidate in the probed top-k pool was Lean-accepted (primary trigger target)
- `selector_top1_accepted` — whether the ExecSelector's chosen candidate was Lean-accepted (diagnostic)
- `label_source = "lean_probe"` — live Lean acceptance, not selector score proxy

**Collection run:**
- 29,186 apply-shaped theorems from leandojo_mathlib, randomized
- 8 parallel shards, each spawning its own Pantograph server
- `--probe-lean --probe-k 5` — tests top-5 candidates per trigger state via live Lean
...
|---|---|
| Total rows | 27,181 |
| Trigger rows (lean_probe) | 21,006 |
| Goal-start failures | 6,175 (2,601 repaired via file_context) |
| can_apply=1 | 4,726 (22.5%) |
| selector_top1_accepted=1 | 3,122 (14.9%) |
| Unique theorems | 20,959 |
| Theorems with positives | 4,717 |
| Mean candidates probed/state | 3.8 |
| Pools with >1 accepted | 705 |

**Per-stage breakdown:**

| Stage | Positives | Total | Rate |
|---|---|---|---|
| post_ib_fail | 3,133 | 13,153 | 23.8% |
| post_rw | 399 | 1,734 | 23.0% |
| post_auto | 842 | 4,694 | 17.9% |
| mid_search | 352 | 1,425 | 24.7% |

**Key findings:**
- 22.5% positive rate (vs 5% with prior single-candidate cosine-top-1 probe) — top-k pool approach finds 4.5× more positive states
- All four stages are productive; `mid_search` (24.7%) and `post_ib_fail` (23.8%) are richest
- selector picks correctly 66% of the time when any candidate works (3,122/4,726)
- 705 pools with >1 accepted candidate — confirms probing deeper matters
- Top namespaces by positives: (root) 676, MeasureTheory 480, CategoryTheory 159, Polynomial 127

---

### EXP-051: Apply Trigger Classifier

**Date:** 2026-03-19
**Script:** `scripts/train_trigger_classifier.py`
**Purpose:** Train a binary classifier to predict when the controller should invoke the apply specialist.
**Target:** `can_apply` (any candidate in probed pool Lean-accepted). Auxiliary: `selector_top1_accepted`.

#### v1 (baseline, 394d features)

**Features:** goal_emb(384) + stage_onehot(4) + scalar features(6: open_goals, cosine/selector scores, candidate count, recent_lanes)
**Architecture:** MLP 394→128→64→1, BCEWithLogitsLoss, model selection by val BCE loss.
**Split:** theorem-grouped, 16,768 train / 4,191 val theorems.
...
| Val Precision | 0.379 |
| Val Recall | 0.720 |
| TP/FP/FN/TN | 679/1113/264/2147 |

#### v2 (structured features + aux head, 418d)

**Features:** v1 + goal_shape_features(14d: binder/equality/iff/arrow/forall/exists counts) + namespace_hash(8d) + step(1d) + lane_provenance(1d).
**Architecture:** Shared trunk (418→128→64) + primary head (→1, can_apply) + auxiliary head (→1, selector_top1_accepted, weight=0.3).
**Model selection:** best val F1 (not BCE loss). `pos_weight_scale=0.8`.
**Threshold:** swept on val set.

| Metric | v1 | v2 @0.50 | v2 @0.47 (swept) |
|---|---|---|---|
| Val F1 | 0.497 | 0.508 | 0.511 |
| Val Precision | 0.379 | 0.425 | 0.414 |
| Val Recall | 0.720 | 0.630 | 0.667 |

**Per-stage (v2, threshold=0.47):**

| Stage | Precision | Recall | TP | FP | FN | TN |
|---|---|---|---|---|---|---|
| post_ib_fail | 0.41 | 0.69 | 428 | 626 | 194 | 1376 |
| post_rw | 0.44 | 0.70 | 64 | 81 | 27 | 204 |
| post_auto | 0.40 | 0.57 | 95 | 141 | 73 | 635 |
| mid_search | 0.51 | 0.68 | 42 | 41 | 20 | 156 |

**Per-stage thresholds:** post_ib_fail=0.47, post_rw=0.49, post_auto=0.46, mid_search=0.45. Narrow range — stage-specific calibration gives mild gains.

**Key findings:**

1. **The trigger classifier is operationally useful.** At 67% recall / 41% precision, it catches most apply-amenable states at a cost of ~2.4 wasted apply calls per real success — acceptable given apply costs ~1 Lean call.
2. **Structured features help.** goal_shape_features + namespace_hash + step lifted F1 by +1.4pp and shifted the operating point toward precision (+4.6pp) at modest recall cost (−5.3pp).
3. **Auxiliary selector_top1_accepted head did not hurt.** Provides useful controller signal (is the selector trustworthy at this state?) without degrading the primary target.
4. **F1-based model selection > BCE loss selection.** Directly optimizing the deployment metric produces better threshold behavior.
5. **pos_weight_scale=0.8** (slightly reduced positive weighting) bought precision without losing usefulness.

**Checkpoint:** `models/apply_trigger_v2.pt` (418d input, trunk+heads, threshold=0.47).
**Embedding cache:** `data/trigger_goal_embs.npy` (21,006×384d, enables fast iteration).

**Decision:** v2 is the deployed trigger classifier. Next: wire into proof_search.py, replace `_apply_gate`, rerun 50-theorem benchmark to measure theorem-level gain.

#### v3 (runtime-compatible, 414d — no selector features)

v2's features included `selector_top1_score` and `cosine_top1_score_positive`, which are not available at trigger time (trigger fires *before* candidate ranking). v3 removes these 4 features, keeping only pre-ranking information.

| Metric | v2 (418d) | v3 (414d) |
|---|---|---|
| Val F1 @swept | 0.511 | 0.422 |
| Val Precision | 0.414 | 0.306 |
| Val Recall | 0.667 | 0.680 |
| Best threshold | 0.47 | 0.47 |

F1 drops 9pp without selector features — expected, those features carried real signal. But v3 is the only version that can deploy at runtime. Checkpoint: `models/apply_trigger_v3.pt`.

---

### EXP-051b: Theorem-Search Integration with Learned Trigger

**Date:** 2026-03-19
**Script:** `scripts/run_exp049_selector_search.py` (condition: `trigger_apply`)
**Purpose:** Test whether the learned trigger classifier + ExecSelector produces real apply-lane activity in theorem search.
**Setup:** 50-theorem Mathlib benchmark (same split as EXP-3.2). Trigger replaces the old deterministic `_apply_gate` (which fired 0 times in EXP-049).

**Conditions:**

| Condition | Description |
|---|---|
| baseline | IB only, no sentence encoder, no apply |
| trigger_apply | IB + cosine_rw + cosine_apply_gated (learned trigger v3 + ExecSelector v1) |

**Results:**

| Condition | Proved | Apply-lane goal closes | Time |
|---|---|---|---|
| baseline | 13/50 (26%) | 0 | 41s |
| trigger_apply | 14/50 (28%) | **1** | 23s |

**Apply lane activity:**

- Trigger fired on multiple theorems across the benchmark (vs 0 fires with old `_apply_gate`)
- `exec_selector_apply` closed 1 goal: `SimpleGraph.degree_le_maxDegree`, step 34, tactic `apply Finset.le_max_of_eq`
- Full close provenance for that theorem: `interleaved_bootstrap → cosine_rw → cosine_rw → exec_selector_apply → interleaved_bootstrap` (5-step proof, apply contributed step 4)
- Theorem did not fully prove (remaining goals), but the apply lane made real, validated progress
- The +1 net theorem (`WeierstrassCurve.Jacobian.addU_of_Z_eq_zero_right`) came from cosine_rw (sentence encoder enabled in trigger_apply condition), not from apply directly
- Zero regressions

**Key findings:**

1. **The learned trigger successfully activates the apply lane in theorem search.** The old deterministic `_apply_gate` produced exactly 0 fires on this benchmark; the learned trigger produces real activations and real goal closes.
2. **The apply executor works end-to-end in search.** The trigger fires → ExecSelector picks a candidate → Lean accepts it → goal closes. The full pipeline is operational.
3. **The +1 theorem is from cosine_rw, not apply.** The apply lane's goal-close on `SimpleGraph.degree_le_maxDegree` did not translate to a theorem close because other goals remained. Apply is now a productive *helper* lane (like simp), not yet a *finisher* lane.
4. **No regressions.** The trigger's false positives cost ~1 Lean call each, well within budget.

**Decision (frozen):** The trigger + selector + apply pipeline is deployed and operational. The apply lane is now a productive helper lane in theorem search. The orchestration bottleneck identified in EXP-049 is resolved.

---

### EXP-051c: Clean A/B and Selector Beam Ablation

**Date:** 2026-03-19
**Script:** `scripts/run_exp049_selector_search.py`
**Purpose:** (1) Clean same-stack A/B isolating apply lane effect. (2) Beam-width ablation to test whether selector is "almost right" or structurally mismatched.

**Setup:** 50-theorem Mathlib benchmark. Both conditions use NAV-004 + IB + cosine_rw. Only difference: trigger_apply adds learned trigger (v3, threshold=0.65) + ExecSelector v1.

**Instrumentation added:**
- Cache-aware trigger: check one-shot cache *before* running trigger MLP (eliminates 75/87 wasted fires from prior run)
- Per-fire diagnostics: `accessible_count`, `apply_candidates` with Lean outcomes, `trigger_prob`
- Apply-specific scoping: direct cosine ranking over all accessible premises (removed `scope_for_rw` for apply family)

**Results (beam=1 vs beam=3):**

| Condition | Proved | Trigger fires | Real attempts | Lean accepts |
|---|---|---|---|---|
| cosine_rw_only | 14/50 | 0 | 0 | 0 |
| + trigger_apply (beam=1) | 14/50 | 12 | 12 | 1 |
| + trigger_apply (beam=3) | 14/50 | 12 | 12 | 1 |

Zero regressions in both. Apply goal-closes: 1 (same goal, `SimpleGraph.degree_le_maxDegree` via `apply Finset.le_max_of_eq`).

**Per-fire diagnostic (beam=3, 12 real fires):**

| Theorem | Prob | Accessible | Candidates tried | Any accepted |
|---|---|---|---|---|
| Cardinal.mk_preimage_of_subset_range | 0.653 | 2 | 2 | No |
| tendsto_prod_nat_add | 0.652 | 12 | 3 | No |
| ENNReal.iInter_Ioi_coe_nat | 0.680 | 4 | 3 | No |
| ENNReal.rpow_le_self_of_le_one | 0.674 | 2 | 2 | No |
| MulPosReflectLT.toMulPosStrictMono | 0.666 | 2 | 2 | No |
| Pi.hasProd | 0.658 | 3 | 3 | No |
| Set.Countable.substructure_closure | 0.766 | 4 | 3 | No |
| Polynomial.natDegree_prod' | 0.656 | 2 | 2 | No |
| Cardinal.powerlt_aleph0_le | 0.665 | 6 | 3 | No |
| FirstOrder.Language.PartialEquiv.cod_le_cod | 0.735 | 8 | 3 | No |
| **SimpleGraph.degree_le_maxDegree** | **0.703** | **5** | **1** | **Yes** |
| SimpleGraph.degree_le_maxDegree (2nd goal) | 0.735 | 5 | 3 | No |

**Key findings:**

1. **Beam 1→3 adds zero accepts.** The selector's top-3 are semantically adjacent but not unification-compatible. The selector is ranking semantic relatedness, not executable compatibility.
2. **Accessible premise pools are small but nonzero.** Median 4 (range 2-12). No theorem-ID lookup failures.
3. **Trigger is cache-aware and tight.** 12 fires = 12 real attempts. No wasted evaluations.
4. **The bottleneck is now the selector, not the trigger or scoping.** The trigger correctly identifies apply-amenable states. The scoping finds related premises. But the selector cannot distinguish "semantically similar" from "unification-compatible" within these small pools.

**Diagnosis:**
- ExecSelector v1 was trained on 9,231 step-0 canonical probes with features `[goal_emb; cand_emb; cosine_score; filter_passed]`. The `filter_passed` feature is hard-coded to 1.0 at runtime (collapsed signal). The model has no structural compatibility features.
- The training distribution (canonical step-0) differs from the runtime distribution (post-bootstrap/post-rw search residuals).
- The objective (pointwise binary P(executable)) does not incentivize correct ranking within a small pool — it only needs to separate executable from non-executable globally.

**Decision (current):** Stop iterating on trigger and beam width. The next experiment is a residual-state apply selector with compatibility features.

---

### EXP-051d: ExecSelector v2 (Compatibility Features)

**Date:** 2026-03-19
**Script:** `scripts/train_apply_exec_selector_v2.py`
**Purpose:** Test whether structural compatibility features improve the selector's ability to distinguish "semantically similar" from "unification-compatible."

**Architecture:** v1's 770d features + 26d compatibility features = 796d.
New features: goal_head/candidate_head match (1d), goal_head onehot (8d), candidate_head onehot (8d), goal/cand binder counts (2d), local-name overlap (1d), goal shape booleans (5d), candidate arity (1d).

**Training data:** 22,209 rows (9,231 step-0 + 12,978 midstep probes). Grouped split by theorem.

**Offline results:**

| Metric | v1 (770d, 9K step-0) | v2 (796d, 22K combined) |
|---|---|---|
| Val PR-AUC | 0.713 | 0.682 |
| Model top-1 (grouped by theorem+goal) | 38.5% (91 groups) | **45.4%** (368 groups) |
| Cosine top-1 | 16.5% | 20.4% |
| Model top-3 | — | **51.4%** |
| Cosine top-3 | — | 31.5% |

v2 top-1 is 45.4% on a harder, larger eval set — 2.2× cosine lift. Top-3 at 51.4%.

**Runtime results (50-theorem benchmark):**

| Metric | v1 beam=3 | v2 beam=3 |
|---|---|---|
| Trigger fires | 12 | 12 |
| Lean accepts | 1 | 1 |
| Candidates changed | — | 7/12 different picks |

v2 selects different candidates on 7/12 fires — the compat features shift rankings. But it **lost** the one v1 success (`SimpleGraph.degree_le_maxDegree`: v1 picked `Finset.le_max_of_eq` ✓, v2 picked `SimpleGraph.maxDegree` ✗).

**Root cause:** Same train/deploy feature mismatch as trigger v2→v3. The compat features that help offline (candidate conclusion head, candidate binder count, candidate arity, head_match, local-name overlap) require the candidate's type signature at runtime. The DB does not store type signatures, and calling `env_inspect` per candidate is too expensive. At runtime, these features are zero-filled, so the model's learned weights on them produce noise instead of signal.

**Diagnosis:**
1. **Offline, compat features help significantly** (45.4% vs ~38.5% top-1). The features are informative when populated.
2. **At runtime, only goal-side features are available** (goal_head, goal binders, goal shape bools). Candidate-side compat features collapse to zero.
3. **The fix is one of:** (a) populate candidate types at runtime via `env_inspect` caching (one call per unique candidate name, amortized across theorems), (b) retrain with goal-only compat features (same pattern as trigger v3), or (c) embed candidate type signatures in the DB at data pipeline time.

**Decision:** Option (c) is the cleanest — add a `type_pp` column to the entities table during `build_proof_network_db.py`. This is a one-time data pipeline change that makes candidate types available at O(1) cost for all downstream selectors. Until then, deploy v1 at runtime (it picks the right candidate on the one successful fire) and use v2 offline only.

---

### EXP-3.3: Mathlib Test Split (500-theorem slice)

**Date:** 2026-03-19
**Script:** `scripts/run_exp049_selector_search.py`
**Theorems:** 500 random from leandojo_mathlib, disjoint from 50-theorem dev set (`data/mathlib_test_500.jsonl`)
**Checkpoint:** NAV-004

**Important attribution note:** The v2 selector (796d, with compat features) failed to load during this run due to a dimension mismatch in the inline loader — all accepted apply steps used cosine ranking, not the trained selector. The loader has been fixed and v2 loading confirmed in subsequent smoke tests. A clean v2 rerun is needed for selector attribution.

**Results:**

| Metric | cosine_rw_only | + trigger_apply |
|---|---|---|
| Total | 500 | 500 |
| Started | 413 | 413 |
| Skipped (goal start fail) | 87 (17.4%) | 87 |
| **Proved** | **118 (23.6%)** | **119 (23.8%)** |
| **Proved\|started** | **118/413 (28.6%)** | **119/413 (28.8%)** |
| Trigger fires | — | 91 |
| Trigger rejects | — | 2,433 |
| Apply attempts | — | 91 |
| Apply accepts (Lean OK) | — | 12 (13.2%) |
| Apply goal closes | — | 1 |
| Theorems with apply progress | — | 10 |

**Gained:** +1 (`LinearMap.nilRank_eq_polyCharpoly_natTrailingDegree` via `apply LinearMap.nilRankAux_basis_indep`, trigger_prob ≈ 0.655)
**Lost:** 0

**Key findings:**

1. **+1 theorem at 500-theorem scale, 0 regressions.** The apply lane is additive and produces real theorem-level lift.
2. **13.2% accept rate** (12/91 attempts) — up from 8.3% on the 50-theorem dev set, confirming apply-specific scoping and pool expansion help.
3. **10 theorems had apply progress** — the lane is a real helper, not a single-theorem curiosity.
4. **3.6% trigger fire rate** (91/2524 evaluations at threshold 0.65) — selective at scale.
5. **87/500 (17.4%) start failures** — goal creation remains a significant benchmark caveat. Proved|started (28.6%) is the more honest conditional metric.
6. **One-hop pool expansion active** — disclosed: pools < 10 premises are expanded with one-hop neighbors from the dependency graph. This relaxes strict accessible-premises.

**Strongest honest claim:** A gated cosine-ranked apply lane with learned trigger now gives nonzero theorem-level lift at 500-theorem scale, with selective firing, meaningful accepted applies, and 10 theorems materially affected.

**Not yet claimed:** v2 typed selector caused the lift (loader was broken during this run). Clean v2 validation pending.

---

### EXP-3.3b: Clean v2 Selector Rerun (500 theorems)

**Date:** 2026-03-19
**Script:** `scripts/run_exp049_selector_search.py` (condition: `cosine_rw_trigger_apply`)
**Purpose:** Validate v2 selector at 500-theorem scale with confirmed loader (v2 checkpoint loads successfully, `exec_selector_apply` provenance verified in smoke test).

**Results (v2 selector, trigger v3 @ 0.65, beam=3):**

| Metric | Prior (cosine fallback) | v2 selector (this run) |
|---|---|---|
| Proved | 119/500 (23.8%) | 118/500 (23.6%) |
| Started | 413 | 413 |
| Trigger fires | 91 | 101 |
| **Apply accepts** | **12 (13.2%)** | **32 (31.7%)** |
| Apply goal closes | 1 | 0 |
| Theorems with apply progress | 10 | 19 |
| Gained vs baseline | +1 | 0 |

**Key findings:**

1. **Accept rate 13.2% → 31.7%.** The v2 selector with real candidate types (from `entities.type_pp`) more than doubles the Lean acceptance rate. The compat features (head match, binder counts, candidate arity, local-name overlap) are working at runtime.
2. **19 theorems with apply progress** (up from 10). The selector finds valid apply steps on nearly twice as many theorems.
3. **0 goal closes, 0 gained theorems.** Despite better step-level quality, the v2 selector's accepts open subgoals rather than closing goals. The +1 theorem from the prior run was a cosine hit, not a selector hit.
4. **32 `exec_selector_apply` closes in provenance** — confirms the v2 selector is running and selecting candidates, not falling back to cosine.

**Interpretation:** The v2 selector is a materially better component (31.7% vs 13.2% accept rate), but the apply lane's theorem-level ceiling is limited by the fact that `apply` usually opens subgoals rather than closing goals. The path from helper to finisher is multi-step planning, not better single-step selection.

**Decision:** v2 selector is the deployed apply selector. The accept rate doubling validates the compatibility feature approach. Next bottleneck: multi-step proof planning where apply contributes one step in a longer proof.

---

### EXP-3.3c: Goal-Start Repair + Refine + v2 Selector (500 theorems)

**Date:** 2026-03-20
**Script:** `scripts/run_exp049_selector_search.py`
**Purpose:** Full 500-theorem benchmark with three improvements: (1) Tier B file-context goal creation fallback, (2) `exact`/`refine ?_ ?_ ?_` variants before bare `apply`, (3) v2 selector with compat features.

**Key infrastructure change:** `_resolve_initial_goal` now falls back to `lean.goal_via_file_context()` (Tier B cascade) when `goal_start` fails. Uses `env_inspect` to find the theorem's source module, reads the declaration, wraps with active scope context, and loads via `load_sorry`. Handles universe-polymorphic types and coercion issues.

**Results:**

| Metric | Prior (EXP-3.3) | This run (EXP-3.3c) |
|---|---|---|
| **Started** | **413/500** | **454/500** |
| **Skipped** | **87 (17.4%)** | **46 (9.2%)** |
| **Proved (baseline)** | **118 (23.6%)** | **135 (27.0%)** |
| **Proved\|started** | **118/413 (28.6%)** | **135/454 (29.7%)** |

| Apply detail | Prior (EXP-3.3b) | This run |
|---|---|---|
| Trigger fires | 101 | 107 |
| Apply accepts | 32 (31.7%) | 37 (34.6%) |
| Theorems with apply progress | 19 | 23 |
| Apply goal closes | 0 | 0 |
| Net theorem delta from apply | 0 | 0 |

**Gained:** +17 theorems vs prior baseline, 0 lost.

**Key findings:**

1. **On the current 500-theorem benchmark, startability repair is a larger lever than any tested model-side change so far: +17 theorems.** Skips 87→46, giving 41 more started theorems, 17 of which prove with existing lanes.
2. **Skip rate halved: 17.4% → 9.2%.** Tier B file-context cascade handles elaboration that pretty-printed types can't.
3. **Proved\|started also improved: 28.6% → 29.7%.** Some newly-started theorems are easier than the prior distribution.
4. **Apply modestly improved: 37 accepts (34.6%).** Refine variants find a few more valid steps. Still 0 goal closes, 0 theorem delta from apply.
5. **Remaining 46 skips break down as:** 19 symbol-not-found (data staleness — leandojo IDs vs current Mathlib v4.27.0, should be excluded from denominator) + 27 genuine Tier B elaboration failures (universe-polymorphic types, typeclass resolution, complex coercions).

**Corrected denominator:** Excluding the 19 stale IDs, the effective benchmark is 481 theorems, of which 454 start (94.4%) and 135 prove (28.1% overall, 29.7% of started).

**Decision:** Tier B goal creation is the default fallback in the canonical benchmark path. Future experiments that do not use this repaired-start baseline are stale. The remaining 27 elaboration failures are a tracked dataset (`data/skip_analysis_500.json`) for the ContextIR improvement stream.


### EXP-3.3e: 2000-Theorem Benchmark (Main Status Line)

**Date:** 2026-03-21
**Script:** `scripts/run_exp049_selector_search.py` with 5-minute per-theorem timeout
**Theorems:** 2000 random from leandojo_mathlib (`data/mathlib_test_2000.jsonl`), superset of 500-theorem dev slice
**Configuration:** NAV-004 + Tier B goal creation + ContextIR++ + v2 selector + trigger v3 + refine variants

**Results:**

| Metric | cosine_rw_only | + trigger_apply |
|---|---|---|
| **Proved** | **557/2000 (27.9%)** | **560/2000 (28.0%)** |
| **Proved\|started** | **557/1759 (31.7%)** | **560/1759 (31.8%)** |
| Started | 1759/2000 (88.0%) | 1759/2000 |
| Skipped | 241 (12.1%) | 241 |
| Trigger fires | — | 484 |
| Apply accepts | — | 164 (33.9%) |
| **Apply goal closes** | — | **2** |
| Theorems with apply progress | — | 100 |
| Time | 6320s | 7566s |

**Gained:** +3 (`PrimeMultiset.coePNat_nat`, `Multiset.count_sum'`, `MeasureTheory.Measure.AbsolutelyContinuous.restrict`). Lost: 0.

**Apply goal closes (first theorem-level apply milestone):**

1. **`Multiset.count_sum'`** — proved in 12 attempts. Sole closer: `exec_selector_apply`. One-step proof via apply.
2. **`PrimeMultiset.coePNat_nat`** — proved in 48 attempts. Path: IB → cosine_rw → cosine_rw → `exec_selector_apply` (final closer). Apply as the finishing move in a multi-lane proof.

**Key findings:**

1. **First theorem-level benefit from a learned specialist beyond the rw substrate.** Two theorems directly closed by `exec_selector_apply`. This is the first measured evidence that the apply lane contributes to theorem completion, not just goal-level progress.
2. **100 theorems with apply progress at scale** — the lane is a substantial helper. 164 accepts from 484 fires (33.9%) is consistent across all benchmark sizes.
3. **31.7% proved|started** — the best conditional rate measured. The system proves nearly 1 in 3 theorems it can start.
4. **Cost: +20% runtime** (6320s→7566s). The apply lane adds meaningful overhead. Optimization target for deployment.
5. **241 skips (12.1%)** — startability remains a larger lever than apply for theorem count. Each 1% reduction in skip rate adds ~18 potential theorems.

**Claim status update:**
- **Claim A (cheap proof solving):** Demonstrated strongly. 560 Mathlib theorems on a laptop.
- **Claim B (SoM composition):** Early composition demonstrated. No longer aspirational — first end-to-end specialist benefit validated.

**Decision:** This is the main status line for the pre-exact? baseline.

---

### EXP-055: Last-Resort `exact?` Closer

**Date:** 2026-03-21
**Script:** `scripts/run_exp049_selector_search.py` + `exact?` in `_EXPENSIVE_CLOSERS`
**Purpose:** Test `exact?` as a last-resort closer when exactly 1 goal remains and all other lanes have failed. Uses Lean's own search as a teacher/oracle.

**Implementation:** Added to `_search_step` in `proof_search.py`: after all lanes fail and `len(open_goals) == 1`, try `exact?` once (cached per goal text). Bypasses the one-shot structural cache.

**Results (500-theorem benchmark):**

| Metric | Without exact? (EXP-3.3c) | With exact? (EXP-055) |
|---|---|---|
| **Proved** | **135/500 (27.0%)** | **279/500 (55.8%)** |
| **Proved\|started** | **135/454 (29.7%)** | **279/441 (63.3%)** |
| Runtime | 1727s | 2009s (+16%) |

**On 29 near-miss theorems (1 remaining goal after apply progress):** 9/29 proved (31.0%). All 9 use `last_resort_exact` as final closer, 6/9 with `exec_selector_apply` in provenance.

**On 50-theorem dev set:** 14→29 proved (+107%).

**Key findings:**

1. **`exact?` as last-resort closer more than doubles the prove rate.** +144 theorems on 500, at only 16% runtime overhead. This is the largest single improvement in the project.
2. **The bottleneck was argument construction, not premise retrieval or routing.** `exact?` succeeds because it does Lean's own unification-based search — finding the right lemma AND the right arguments simultaneously.
3. **Multi-lane proof composition is now demonstrated.** 9 near-miss theorems prove via: apply opens subgoals → other lanes close most → `exact?` closes the last. This is the first robust multi-step composition result.
4. **`exact?` is used as teacher/oracle.** It is NOT the deployed runtime lane — it's the upper bound for a learned closer. The next experiment is building a cheaper equivalent.

**Proved theorems from near-miss set (apply + exact? composition):**
- `Polynomial.eraseLead_add_of_degree_lt_left` (apply→apply→exact?)
- `ENNReal.limsup_sub_const` (apply→apply→exact?)
- `FormalMultilinearSeries.comp_assoc` (apply→apply→exact?)
- `IsPrimitiveRoot.card_rootsOfUnity'` (rw→auto→exact?)
- `List.dropSlice_eq_dropSliceTR` (apply→apply→exact?)
- `finprod_option` (apply→IB→exact?)
- `Real.cosh_sub` (IB→apply→exact?)
- `Real.rpow_zpow_comm` (IB→apply→exact?)
- `Finsupp.embDomain_mapRange` (apply→apply→exact?)

**Decision:** `exact?` establishes the ceiling for a final-goal closer. The next experiment (EXP-056) should: (1) harvest `exact?` solutions as training data, (2) build a cheaper learned closer that approximates its behavior, (3) integrate into the 2000-theorem benchmark.

---

### EXP-056: Exact? Solution Harvest and Distillation Dataset

**Date:** 2026-03-21
**Script:** `scripts/run_exp049_selector_search.py` with solution capture in `proof_search.py`
**Purpose:** Capture the exact tactics that `exact?` finds, to build training data for a learned closer.

**Results (500-theorem harvest):**

- 317/500 proved (63.4%) — higher than prior 279/500 due to solution capture giving search a second chance
- **195 captured `exact?` solutions** with full (goal_state, closing_tactic) pairs
- Dataset: `data/closer_training_500.jsonl`

**Solution profile:**
- 185/195 (95%) are self-applications: `exact TheoremName local_args`
- 10 (5%) use a different lemma (variants, synonyms, helper lemmas)
- Argument count: 0 args (7%), 1 arg (33%), 2 args (26%), 3+ args (34%)
- All 195 solutions fire on modified goals (post-search), not raw theorem types

**Key insight:** Self-application dominance (95%) means the system reduces theorems until they can be proved by applying themselves with the right arguments from local context. The infrastructure does the reduction. Lean's unifier does the argument instantiation.

---

### EXP-057: Regression Audit — Apply Specialist Under Strong Finisher Regime

**Date:** 2026-03-21
**Purpose:** Determine why the apply specialist loses theorems when added on top of the exact? baseline.

**Results (2000-theorem comparison):**

| Configuration | Proved | Delta |
|---|---|---|
| exact? baseline (cosine_rw_only) | 1113/2000 | — |
| + trigger_apply | 1069/2000 | **-44** |

**Gained: 0. Lost: 44. Net: -44.**

All 44 losses are `apply_accepted_but_nonproductive`:
- Apply fires, Lean accepts → opens subgoals
- Subgoals consume budget (attempts balloon from ~30 to 60-600)
- Stall detector kills search before exact? reaches the 1-goal state
- Baseline would have proved via exact? in ~30 attempts

**Worst cases:** `FormalMultilinearSeries.radius...` (16 fires, 16 accepts, 600 attempts, failed), `IsPreconnected.subset_or_subset` (4 fires, 4 accepts, 296 attempts, failed).

**Lane taxonomy (validated):**
- **rw**: productive substrate
- **exact? / shape / induction / micro**: finisher stack
- **apply**: subgoal-opening specialist — **off by default under strong finisher regimes**

**Decision:** Apply should only fire when the finisher stack has already failed. The correct architecture routes to the cheapest adequate closer, not the most sophisticated one. This is a theory result: more model at the wrong layer is actively harmful.

---

### EXP-058: Decisive 2000-Theorem Benchmark (All Improvements, No Apply)

**Date:** 2026-03-21
**Script:** `scripts/run_exp049_selector_search.py`
**Configuration:** NAV-004 + cosine_rw + IB + automation + exact? last-resort + goal-shape-conditioned tactics + induction lanes + micro-tactics + simp_all. **No apply lane.**

**This is the current best result and the main status line.**

**Results:**

| Metric | Value |
|---|---|
| **Proved** | **1,277/2,000 (63.8%)** |
| **Proved\|started** | **1,277/1,759 (72.6%)** |
| Started | 1,759/2,000 (88.0%) |
| Skipped | 241 (12.1%) |
| Runtime | 4,008s |

**Comparison across configurations:**

| Configuration | Proved/2000 | Rate | Time |
|---|---|---|---|
| Infrastructure only (no exact?) | 560 | 28.0% | 6320s |
| + exact? | 1113 | 55.7% | 8040s |
| + exact? + apply | 1069 | 53.5% | 9292s |
| **+ exact? + shape + induction + micro (no apply)** | **1277** | **63.8%** | **4008s** |

**Key findings:**

1. **+164 theorems over exact?-only, at half the runtime.** Shape-conditioned tactics (induction on typed variables, case-split on disjunctions, micro-closers) add 164 theorems AND reduce runtime from 8040s to 4008s by closing theorems faster than exact?.
2. **72.6% proved|started** — the system proves nearly 3 in 4 theorems it can start.
3. **The finisher stack dominates.** exact? + structural heuristics is strictly better than exact? + learned specialist. The cheap finisher stack handles 63.8% of Mathlib; the apply specialist is counterproductive in this regime.
4. **Runtime halved** compared to exact?-only — the structural tactics close many theorems in <1s that exact? would need 5-10s for, because they pattern-match on goal structure instead of running Lean's full search.

**Decision (frozen):** This is the canonical result. The finisher stack (exact? + shape + induction + micro) is the deployed configuration. Apply is off by default and should only be reintroduced with dominance-aware routing (fire only after finisher stack fails). The 2000-theorem benchmark at 63.8% is the publishable headline.
...
- Zero is majority value for 5/6 banks (Informational Zero confirmed)
- 729-bin direction space is very sparse (17/729 = 2.3% occupied)
...
| H54 | rw0 learned decoder beats cosine baseline on LeanValid@rw0                                                                                           | started with real premise scope | Refuted (current)                                                                                                                                                                                                                                                                                   | Same-denominator step-0 benchmark: learned top-5 38.5% < cosine top-5 61.5%; learned top-1 3.8% < cosine top-1 33.3%. Cosine is the deployed rw0 baseline; learning shifts to harder tiers and reranking. |
...
| 2026-03-10 | Narrative regime conversion via template classification                                                                                                                                                      | Specification complexity Theorem 4.1:                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | G_μ                                                                                                                                                                                                          |
...
| 2026-03-19 | Promote goal startability / context repair to a first-class specialist and dataset track.                                                                                                                    | Large trigger-collection shards show hundreds of `Could not create goal` failures concentrated in high-context namespaces (`CategoryTheory`, `MeasureTheory`, `Submodule`, `Filter`, etc.). These are not mainly lexical-name issues; they are theorem-site compilation failures. Dropping them biases trigger and executable-selector datasets toward already-startable goals. The collector now writes explicit `goal_start_failure` rows with `ContextIR` features, goal-shape summaries, structured `LeanFeedback`, and repair outcomes. | Keep skipping non-startable theorems in trigger collection; treat them as generic negatives or as infrastructure noise outside the SoM boundary                                                              |
