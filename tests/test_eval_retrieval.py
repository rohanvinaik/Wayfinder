...
class TestComputeRecallAtK(unittest.TestCase):
    ...
    def test_zero_recall(self):
        self.assertAlmostEqual(
            compute_recall_at_k(["x", "y"], ["a", "b"], k=2), 0.0
        )

    def test_partial_recall(self):
        self.assertAlmostEqual(
            compute_recall_at_k(["a", "x"], ["a", "b"], k=2), 0.5
        )

    def test_k_limits_retrieved(self):
        # Only first k=1 retrieved item counts
        self.assertAlmostEqual(
            compute_recall_at_k(["x", "a"], ["a"], k=1), 0.0
        )
...
class TestComputeUniverseCoverage(unittest.TestCase):
    ...
    def test_full_coverage(self):
        cov, covered, uncovered = compute_universe_coverage(
            ["a", "b"], {"a", "b", "c"}
        )
    ...
    def test_preserves_order(self):
        _, covered, uncovered = compute_universe_coverage(
            ["c", "a", "x"], {"a", "c"}
        )
